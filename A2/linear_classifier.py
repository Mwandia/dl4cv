...
def svm_loss_vectorized(W, X, y, reg):
  ...
  scores = X.mm(W)
  c_scores = scores[range(num_train), y].view(-1,1) 
  margins = scores - c_scores + 1
  ...
  #############################################################################
  # Replace "pass" statement with your code
  big_margins = (margins > 0).type(X.dtype)
  big_margins[range(num_train), y] -= big_margins.sum(dim=1)
  dW = X.T.matmul(big_margins)
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  indices = torch.randint(low=0,high=batch_size,size=(batch_size,))
  X_batch = X[indices]
  y_batch = y[indices]
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = X.matmul(W).max(dim=1).indices
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  # Replace "pass" statement with your code
  learning_rates = [1e-2]
  regularization_strengths = torch.logspace(-0.03,-0.02,50)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = torch.mean((data_dict['y_train'] == cls.predict(data_dict['X_train'])).double())
  val_acc = torch.mean((data_dict['y_val'] == cls.predict(data_dict['X_val'])).double())
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
