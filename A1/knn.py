...
def compute_distances_two_loops(x_train, x_test):
  ...
  dists = x_train.new_zeros((num_train, num_test),dtype=x_train.dtype)
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  # Replace "pass" statement with your code
  for j in range(num_test):
    indices = dists[:, j].topk(k, largest=False).indices
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for idx in range(num_folds):
      ...
      elif idx == num_folds - 1:
        x_Train = torch.cat(x_train_folds[:idx])
        y_Train = torch.cat(y_train_folds[:idx])
      else:
        x_Train = torch.cat((torch.cat(x_train_folds[:idx]),torch.cat(x_train_folds[idx+1:])))
        y_Train = torch.cat((torch.cat(y_train_folds[:idx]),torch.cat(y_train_folds[idx+1:])))
      
      x_Test, y_Test = x_train_folds[idx], y_train_folds[idx]
      classifier = KnnClassifier(x_Train,x_Train)
...
def knn_get_best_k(k_to_accuracies):
  ...
  ks = []
  means = []
  for i, acc in sorted(k_to_accuracies.items()):
    ks.append(i)
    means.append(statistics.mean(acc))

  best_k = ks[means.index(sorted(means)[-1])]
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
