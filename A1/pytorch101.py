...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  step_size = 10
  nearest_10 = (round(start/step_size) * step_size) if round(start/step_size) > 0 else 10
  x = torch.arange(nearest_10, stop, step_size) if nearest_10 < stop else torch.tensor([])
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  rows = torch.arange(x.shape[0]).tolist()[::-1]
  y = x[rows,:]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
